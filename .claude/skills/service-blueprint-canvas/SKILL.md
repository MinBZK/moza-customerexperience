...
- **Tijdmarkers**: bredere connector `.step-connector--gap` met `data-gap-label="na X"` op dezelfde kolom in elke rij die die kolom haalt; JS tekent één verticale band met klok per unieke x-positie. De band stopt bóven de Vraagstukken-rij: tijdmarkers horen bij de tijdlijn-rijen en mogen nooit door de vraagstukken-content lopen.
...
- **Vraagstukken-rij**: onder de Systeem-rij(en) staat een rij "Vraagstukken" die per fase automatisch de vraagstukken verzamelt uit de zijpaneel-templates (knoppen met `data-vraagstukken-ref` per `data-phase`, gededupliceerd per fase) - zelfde bron als de blauwe overlay, dus altijd synchroon. Elk vraagstuk wordt als box (`.vr-item`, 168px breed zoals een stap) op de x-positie van de bijbehorende stap gezet, zodat het verticaal uitlijnt met de stap waar het bij hoort; boxen op dezelfde kolom stapelen, de cel krijgt via JS een passende min-height. Komt een vraagstuk bij meerdere stappen in dezelfde fase voor, dan blijft het één regel met een stapvermelding erachter ("- speelt bij: <stap> en <stap>", gedempt), nooit dezelfde tekst dubbel. Onder de fase-cellen staat een extra rij "Algemeen" (cel met `grid-column: 2 / -1`, inhoud sticky-links) die de generieke vraagstukken uit `#sidePanelVraagstukkenAlgemeen` overneemt - zo staat álle vraagstukken-content uit de overlay ook in de rij. De content past altijd binnen de kolom: de cel heeft `contain: inline-size` zodat de vraagstukken de kolom nooit breder maken dan de stappen nodig hebben (geen extra witruimte rond de boxen), en de lijst vult de beschikbare breedte tot een leesbaar maximum (`max-width: 520px`, `overflow-wrap: break-word`). Amber accent zoals het blok in het zijpaneel (in licht thema een donkerder amber voor contrast).
