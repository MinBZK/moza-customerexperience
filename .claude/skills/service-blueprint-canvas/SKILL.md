...
- **Vraagstukken-footer**: de vraagstukken staan in een in-/uitklapbare sticky footer onderin het canvas (`.vr-footer`, `position: sticky; bottom: 0` binnen de viewport; breedte = grid-breedte via JS). Ingeklapt (default) toont een compacte balk met amber titel + teller en **amber markers op de kolommen waar vraagstukken zitten**; uitgeklapt staan de vraagstukken als boxen (`.vr-item`, 168px zoals een stap) op de x-positie van hun stap - per fase gededupliceerd, bij herhaling één regel met stapvermelding ("- speelt bij: <stap> en <stap>", gedempt), nooit dezelfde tekst dubbel - plus een "Algemeen"-blok (sticky-links) met de generieke vraagstukken uit `#sidePanelVraagstukkenAlgemeen`. Zelfde bron als de blauwe overlay, dus altijd synchroon. Meet box-hoogtes met de footer tijdelijk zichtbaar (`.meten` = visibility hidden), anders zijn ze 0. **De footer hoort op álle blueprint-pagina's, ook op varianten zoals `api.html`.**
