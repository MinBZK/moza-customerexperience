...
- **Tijdmarkers**: bredere connector `.step-connector--gap` met `data-gap-label="na X"` op dezelfde kolom in elke rij die die kolom haalt; JS tekent één verticale band met klok per unieke x-positie. De band loopt tot de onderkant van het grid; de vraagstukken zitten in de sticky footer buiten het grid en worden dus nooit door de band gekruist.
...
- **Vraagstukken-footer**: de vraagstukken staan in een in-/uitklapbare sticky footer onderin het canvas (`.vr-footer`, `position: sticky; bottom: 0` binnen de viewport; breedte = grid-breedte via JS). De héle headerbalk is één full-width `<button>` (`.vr-footer-toggle`, klikken werkt dus overal op de rij); titel + totaalteller zitten in een sticky-links `.vr-footer-head`. Ingeklapt (default) staan de **amber kolom-tellers** (`.vr-ind`) als absolute badges ín die knop, verticaal gecentreerd (`top: 50%; translateY(-50%)`) zodat ze exact uitlijnen met de hoofdbadge en nooit onder de rand vallen - niet onderin de footer hangen. Uitgeklapt staan de vraagstukken als boxen (`.vr-item`, 168px zoals een stap) op de x-positie van hun stap - per fase gededupliceerd, bij herhaling één regel met stapvermelding ("- speelt bij: <stap> en <stap>", gedempt), nooit dezelfde tekst dubbel. Het "Algemeen"-blok (generieke vraagstukken uit `#sidePanelVraagstukkenAlgemeen`) staat in de vrije labelkolom (eerste 240px) direct onder de titel (`.lane--vr-algemeen--col`), mits geen box vóór x=244 begint - anders als fallback onder de boxen; het mag nooit een specifiek vraagstuk overlappen. Zelfde bron als de blauwe overlay, dus altijd synchroon. Meet box-hoogtes met de footer tijdelijk zichtbaar (`.meten` = visibility hidden), anders zijn ze 0. **De footer hoort op álle blueprint-pagina's, ook op varianten zoals `api.html`.**
- **Variantswitch in volledig scherm**: navigeren zou fullscreen verbreken (en opnieuw aanzetten mag pas na een klik), dus de variant-toggle laadt de andere variant dan in een `iframe.variant-frame` dat het fullscreen-element vult, met `#embedfs` in de URL. Een klein bootstrap-script direct na `<body>` zet dan `html.embedfs`, waarmee de geladen pagina alleen haar canvas schermvullend toont (`position: fixed; inset: 0`), haar fullscreen-knop "Sluit volledig scherm" wordt (die `parent.document.exitFullscreen()` aanroept) en haar toggle-links de `#embedfs`-stand vasthouden. Bij het sluiten van volledig scherm navigeert de bovenliggende pagina alsnog naar de variant die in het iframe in beeld stond (`location.replace`). Fullscreen wordt zo nooit verbroken bij het wisselen.
