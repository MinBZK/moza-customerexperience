...
- **Teksten passen altijd binnen de boxen.** Zet `hyphens: auto; overflow-wrap: anywhere;` op `.step` en `.step-verdieping` (werkt alleen met `lang="nl"` op `<html>`), zodat lange Nederlandse samenstellingen (bv. "gebruikersacceptatietest") netjes afbreken in de smalle boxen (168px). Controleer na tekstwijzigingen op overflow: `el.scrollWidth > el.clientWidth` mag nergens waar zijn.
